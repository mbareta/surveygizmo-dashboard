...
class EdxApi {
  ...
  createAccount(data) {
    ...
    const form = {
      ...
      goals: '',
      honor_code: 'true',
      bio: 'other', // TODO, currently it's a dropdown on edX side, but it's not implemented in SG
      zipcode: data['Postal Code'],
      city: data.City,
      state: data.State === 'Washington, D.C.' ? 'WD' : getStateAbbreviation(data.State),
      company: data['Organization Name'],
      phone_number: data['Submitter Phone Number - please format: xxx-xxx-xxxx'],
    };
  ...
  createAffiliateEntity(req, data) {
    ...
    const form = {
      ...
      address_2: data['Street Address 2'],
      zipcode: data['Postal Code'],
      city: data.City,
      state: data.State === 'Washington, D.C.' ? 'WD' : getStateAbbreviation(data.State),
      phone_number: data['Public Contact Phone number - please format: xxx-xxx-xxxx'],
      phone_number_private: data['Affiliate Phone Number - please format: xxx-xxx-xxxx'],
