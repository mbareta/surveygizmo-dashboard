...
class SurveyGizmo {
  ...
  getResponses(responseId = null) {
    const url = this.buildQuery(responseId).responsesQuery;
    return axios(url)
    .then(response => ({
  ...
  getResponseData(responseId) {
    return this.getQuestions()
      ...
      .then(({ currentPage, pageCount, data }) => {
        if (Array.isArray(data)) {
          return { currentPage, pageCount, data: this.tangleQuestionsAndResponses(data) };
        }
      ...
      .then(({ currentPage, pageCount, data }) => {
        return this.getDataFromDb()
          .then(dbData => {
            if (Array.isArray(data)) {
              return { currentPage, pageCount, data: data.map(response => this.getStatusForResponse(response, dbData)) };
            }
